...
namespace PickelCardGameCP.Logic
{
        ...
        , IMiscDataNM, IStartNewGame
    {
        ...
        {
            var firstItem = thisCol.Where(x => x.Suit == SaveRoot!.TrumpSuit).OrderByDescending(Items => Items.Value).FirstOrDefault();
            if (firstItem != null)
            {
            ...
            {
                EnumRegularCardValueList Nums = 0;
                int firstPlayer;
                firstItem = thisCol.Where(x => x.CardType != EnumRegularCardTypeList.Joker).OrderByDescending(Items => Items.Value).FirstOrDefault();
                if (firstItem != null)
                {
            ...
            return thisCol.Where(x => x.Suit == leadSuit).OrderByDescending(Items => Items.Value).First().Player;
        }
