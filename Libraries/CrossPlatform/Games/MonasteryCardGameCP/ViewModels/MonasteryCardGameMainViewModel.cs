...
namespace MonasteryCardGameCP.ViewModels
{
    ...
    public class MonasteryCardGameMainViewModel : BasicCardGamesVM<MonasteryCardInfo>
    {
        ...
        private async Task MainSets_SetClickedAsync(int setNumber, int section, int deck)
        {
            if (setNumber == 0)
            {
                ToastPlatform.ShowError("You must click on an existing set in order to expand.");
                return;
            }
            ...
            if (thisCol.Count > 2)
            {
                ToastPlatform.ShowError("You cannot select more than 2 cards");
                return;
            }
            if (thisCol.Count == 0)
            {
                ToastPlatform.ShowError("There are no cards selected");
                return;
            }
            ...
            {
                ToastPlatform.ShowError("There needs to be 2 cards selected because this is a double run of three");
                return;
            }
            if (doubles == false && thisCol.Count > 1)
            {
                ToastPlatform.ShowError("Since its not a double run of three, then can only choose one card at a time");
                return;
            }
            int newpos = 0;
            foreach (var thisCard in thisCol)
            {
                ...
                if (newpos == 0)
                {
                    ToastPlatform.ShowError("Sorry, cannot use the card to expand upon");
                    return;
                }
            }
            int x = 0;
        ...
        public async Task CompleteChosenMissionAsync()
        {
            ...
            if (rets == false)
            {
                ToastPlatform.ShowError("Sorry, you did not complete the mission");
                MissionChosen = "";
                return;
            }
