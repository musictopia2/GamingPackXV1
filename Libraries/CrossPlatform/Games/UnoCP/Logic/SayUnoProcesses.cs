    ...
    public class SayUnoProcesses : ISayUnoProcesses
    {
        ...
        async Task ISayUnoProcesses.ProcessUnoAsync(bool saiduno)
        {
            ...
            if (saiduno == false)
            {
                ...
                {
                    ToastPlatform.ShowWarning("You had one card left.  However, you did not say uno.  Therefore, you have to draw 2 cards");
                }
            ...
            if (_gameContainer.SingleInfo!.PlayerCategory != EnumPlayerCategory.Self)
            {
                ToastPlatform.ShowInfo($"Uno From {_gameContainer.SingleInfo.NickName}");
            }
