...
namespace Pinochle2PlayerCP.Logic
{
        ...
        , IMiscDataNM, IStartNewGame
    {
        ...
        public override async Task FinishGetSavedAsync()
        {
            ...
            {
                ToastPlatform.ShowInfo(SaveRoot.StartMessage);
            }
        ...
        public override async Task EndTrickAsync()
        {
            ...
            if (lefts == 0)
            {
                ...
                ToastPlatform.ShowInfo($"{SingleInfo.NickName} gets an extra 10 points for winning the last trick");
                await EndRoundAsync();
                return;
            }
            SingleInfo = PlayerList.GetSelf();
        ...
        public MeldClass GetMeldFromList(IDeckDict<Pinochle2PlayerCardInformation> thisList)
        {
            ...
            if (suitCount == 4 && numberCount == 1 && thisList.Count == 4)
            {
                ...
                {
                    EnumRegularCardValueList.Jack => EnumClassB.J,
                    EnumRegularCardValueList.Queen => EnumClassB.Q,
                    EnumRegularCardValueList.King => EnumClassB.K,
                    EnumRegularCardValueList.HighAce => EnumClassB.A,
                    _ => EnumClassB.None,
                };
                return defaultMeld;
            }
        ...
        public async Task MeldAsync(CustomBasicList<int> thisList)
        {
            ...
            if (thisList.Count == 1)
            {
                ToastPlatform.ShowError("Since the dix was played; can make one more meld");
                await ContinueTurnAsync();
                return;
            }
            if (SingleInfo.PlayerCategory == EnumPlayerCategory.Self)
