...
namespace Pinochle2PlayerCP.ViewModels
{
    ...
    {
        private readonly Pinochle2PlayerMainGameClass _mainGame;
        ...
        protected override async Task ProcessDiscardClickedAsync()
        {
            ...
            {
                ToastPlatform.ShowInfo("Must choose a card from hand in order to exchange for the top card");
                return;
            }
            if (hands + fromTemps > 1)
            {
                ToastPlatform.ShowError("Must choose only one card from hand to exchange");
                return;
            }
            ...
            {
                ToastPlatform.ShowSuccess("Must choose the nine of the trump suit in order to exchange");
                return;
            }
        ...
        public async Task MeldAsync()
        {
            ...
            {
                ToastPlatform.ShowError("Must choose at least one card from hand in order to meld");
                return;
            }
            ...
            {
                ToastPlatform.ShowError("There is no meld combinations here");
                return;
            }
            if (_model.YourMelds.HasSelectedObject())
            {
                foreach (var thisCard in otherList)
                {
                    ...
                    {
                        ToastPlatform.ShowError("Cannot reuse class b for class b");
                        return;
                    }
                    if (tempMeld.ClassCValue == thisMeld.ClassCValue && thisMeld.ClassCValue > EnumClassC.None)
                    {
                        ToastPlatform.ShowError("Cannot reuse a pinochle for a pinochle");
                        return;
                    }
                    if (tempMeld.ClassAValue <= thisMeld.ClassAValue && thisMeld.ClassAValue > EnumClassA.None)
                    {
                        ToastPlatform.ShowError("Cannot download class A to get more points or to create another of same value");
                        return;
                    }
                }
            }
