...
namespace CousinRummyCP.ViewModels
{
    ...
    public class CousinRummyMainViewModel : BasicCardGamesVM<RegularRummyCard>
    {
        ...
        protected override async Task ProcessDiscardClickedAsync()
        {
            ...
            if (counts + others > 1)
            {
                ToastPlatform.ShowError("Sorry, you can only select one card to discard");
                return;
            }
            if (counts + others == 0)
            {
                ToastPlatform.ShowError("Sorry, you must select a card to discard");
                return;
            }
            int index;
            int deck;
        ...
        public async Task FirstSetsAsync()
        {
            ...
            {
                ToastPlatform.ShowError("Sorry; you do not have the required sets yet");
                return;
            }
        ...
        public async Task OtherSetsAsync()
        {
            ...
            if (thisCol.Count == 0)
            {
                ToastPlatform.ShowError("Sorry; you do not have any more sets to put down");
                return;
            }
            CustomBasicList<string> newList = new CustomBasicList<string>();
            await thisCol.ForEachAsync(async thisTemp =>
