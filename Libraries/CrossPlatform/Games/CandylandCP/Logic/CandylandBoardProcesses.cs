...
namespace CandylandCP.Logic
{
    ...
    {
        public CandylandBoardProcesses(TestOptions test, IAsyncDelayer delayer, IEventAggregator aggregator, CommandContainer command)
        {
            _test = test;
            _delayer = delayer;
        ...
        public async Task MakeMoveAsync(int space, CandylandMainGameClass mainGame)
        {
            ...
            for (x = currents - 1; x >= loopTo1; x += -1)
            {
                ...
                _aggregator.RepaintBoard(); //try this instead now.
                //_command.UpdateAll();
                if (_test.NoAnimations == false)
                {
                    await _delayer.DelayMilli(100);
