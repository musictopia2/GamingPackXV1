...
namespace FroggiesCP.Logic
{
    ...
    public class FroggiesMainGameClass : IAggregatorContainer
    {
        ...
        private Dictionary<string, LilyPadModel>? _col_TargetPads = new Dictionary<string, LilyPadModel>();
        private CustomBasicList<LilyPadModel>? _col_RedoPads;
        private LilyPadModel? _obj_SelectedPad;
        public CustomBasicList<LilyPadModel> GetCompleteLilyList()
        {
        ...
        private async Task LeapFrogAsync(LilyPadModel obj_Target, FroggiesMainViewModel model)
        {
            ...
            {
                ToastPlatform.ShowWarning("No Moves Left");
                await NewGameAsync(model);
                model.MovesLeft = NumberOfMoves();
            }
        }
        ...
        {
            _col_RedoPads = new CustomBasicList<LilyPadModel>();
            _col_RedoPads.Clear();
            IEnumerator enum_Pads;
            LilyPadModel obj_TempPad;
            ...
            while (enum_Pads.MoveNext())
            {
                var FirstTemp = (KeyValuePair<string, LilyPadModel>)enum_Pads.Current;
