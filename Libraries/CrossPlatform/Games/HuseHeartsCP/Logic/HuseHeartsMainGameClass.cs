...
namespace HuseHeartsCP.Logic
{
        ...
        , IMiscDataNM, IStartNewGame, IFinishStart
    {
        ...
        protected override async Task ComputerTurnAsync()
        {
            ...
            {
                moveList = SingleInfo!.MainHandList.Where(x => IsValidMove(x.Deck)).Select(x => x.Deck).ToCustomBasicList();
            }
            else
            {
                moveList = _model!.Dummy1!.HandList.Where(x => IsValidMove(x.Deck)).Select(x => x.Deck).ToCustomBasicList();
            }
