...
namespace ClueBoardGameCP.Logic
{
        ...
        : BoardDiceGameClass<ClueBoardGamePlayerItem, ClueBoardGameSaveInfo, EnumColorChoice, int>, IMiscDataNM
    {
        ...
        public async Task MakeAccusationAsync()
        {
            ...
            {
                ToastPlatform.ShowInfo($"The accusation was correct.  {SaveRoot.CurrentPrediction.CharacterName} did it in the {SaveRoot.CurrentPrediction.RoomName} with the {SaveRoot.CurrentPrediction.WeaponName}");
                await ShowWinAsync();
                return;
            }
            if (WhoTurn == MyID)
            {
                ToastPlatform.ShowWarning($"Sorry, the accusation was not correct {SingleInfo.NickName}.  You are out of the game but need to still be there in order to prove other predictions wrong.");
            }
            SingleInfo.InGame = false;
            if (PlayerList.Count(items => items.InGame == true) <= 1)
            {
                ToastPlatform.ShowWarning($"Sorry, nobody got the solution correct.  Therefore, nobody won.  The solution was {Constants.vbCrLf} {SaveRoot.CurrentPrediction.CharacterName} did it in the {SaveRoot.CurrentPrediction.RoomName} with the {SaveRoot.CurrentPrediction.WeaponName}");
                await ShowTieAsync();
                return;
            }
            SaveRoot.GameStatus = EnumClueStatusList.EndTurn;
