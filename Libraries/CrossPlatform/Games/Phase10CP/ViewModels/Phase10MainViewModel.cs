...
namespace Phase10CP.ViewModels
{
    ...
    public class Phase10MainViewModel : BasicCardGamesVM<Phase10CardInformation>
    {
        ...
        public async Task CompletePhaseAsync()
        {
            ...
            {
                ToastPlatform.ShowError("Cannot complete the phase.  Otherwise; there is no card to discard");
                return;
            }
            if (rets == false)
            {
                ToastPlatform.ShowError("Sorry, you did not complete the phase");
                return;
            }
        ...
        protected override async Task ProcessDiscardClickedAsync()
        {
            ...
            if (rets == false)
            {
                if (message != "")
                {
                    ...
                    ToastPlatform.ShowError(message); //because on tablets, its possible it can't show message
        ...
        private async Task MainSets_SetClickedAsync(int setNumber, int section, int deck)
        {
            ...
            {
                ToastPlatform.ShowError("Sorry, the phase must be completed before expanding onto a set");
                return;
            }
