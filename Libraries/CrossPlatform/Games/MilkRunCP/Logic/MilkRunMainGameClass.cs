...
namespace MilkRunCP.Logic
{
    ...
    public class MilkRunMainGameClass : CardGameClass<MilkRunCardInformation, MilkRunPlayerItem, MilkRunSaveInfo>, IMiscDataNM
    {
        ...
        public async Task PlayerPileClickedAsync(MilkRunPlayerItem thisPlayer, PileInfo pileData)
        {
            ...
            if (newDeck == 0)
            {
                ToastPlatform.ShowError("Sorry, must choose a card to play");
                return;
            }
            if (SaveRoot!.CardsDrawn < 2)
            {
                ToastPlatform.ShowError("Sorry, must draw the 2 cards first before playing");
                return;
            }
        ...
        {
            DeckRegularDict<MilkRunCardInformation> output = new DeckRegularDict<MilkRunCardInformation>();
            ...
            DeckRegularDict<MilkRunCardInformation> finalList = new DeckRegularDict<MilkRunCardInformation>();
