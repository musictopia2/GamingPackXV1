...
namespace FluxxCP.ViewModels
{
    public abstract class KeeperActionViewModel : BasicKeeperScreen
    {
        ...
        public async Task ProcessAsync()
        {
            ...
            if (thisList.Count == 0)
            {
                ToastPlatform.ShowError("Must choose a keeper");
                return;
            }
            if (KeeperContainer.Section == EnumKeeperSection.Exchange)
            {
                if (thisList.Count != 2)
                {
                    ToastPlatform.ShowError("Must choose a keeper from yourself and from another player for exchange");
                    return;
                }
                if (ContainsCurrentPlayer(thisList) == false)
                {
                    ToastPlatform.ShowError("Must choose a keeper from your keeper list in order to exchange");
                    return;
                }
            ...
            if (thisList.Count != 1)
            {
                ToastPlatform.ShowError("Must choose only one keeper");
                return;
            }
            ...
            {
                ToastPlatform.ShowError("Cannot steal a keeper from yourself");
                return;
            }
