...
namespace FluxxCP.ViewModels
{
    public class GiveViewModel : BasicSubmitViewModel
    {
        ...
        public override async Task SubmitAsync()
        {
            ...
            {
                ToastPlatform.ShowError("Cannot select any keeper cards because you have to give the cards from your hand");
                _model.UnselectAllCards();
                return;
            }
            if (_model.Goal1!.ObjectSelected() > 0)
            {
                ToastPlatform.ShowError("Cannot select any goal cards because you have to give the cards from your hand");
                _model.UnselectAllCards();
                return;
            }
            ...
            ToastPlatform.ShowError($"Must give {howMany} not {_model.PlayerHand1.HowManySelectedObjects} cards");
            _model.UnselectAllCards();
        }
    }
}