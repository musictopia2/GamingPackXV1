...
namespace BladesOfSteelCP.Logic
{
    ...
    public class FaceoffProcesses : IFaceoffProcesses
    {
        ...
        private async Task AnalyzeFaceOffAsync()
        {
            ...
            if (tempTurn == 0)
            {
                ToastPlatform.ShowInfo("There was a tie during the faceoff.  Therefore; the faceoff is being done again");
            ...
            ToastPlatform.ShowInfo ($"{_gameContainer.SingleInfo.NickName} has won the face off");
            await Task.Delay(2000);
            ClearFaceOff();
