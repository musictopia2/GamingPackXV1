...
namespace BladesOfSteelCP.Logic
{
    ...
    public class BladesOfSteelMainGameClass : CardGameClass<RegularSimpleCard, BladesOfSteelPlayerItem, BladesOfSteelSaveInfo>, IMiscDataNM
    {
        ...
        private async Task StartEndAsync()
        {
            ...
            if (whoWon == 0)
            {
                ToastPlatform.ShowInfo("There was a tie.  Therefore; all the cards are being reshuffled.  Then a faceoff will happen again to see who goes first and the cards are passed out.  Whoever gets the first point in this round wins");
        ...
        public bool CanPlayAttackCards(IDeckDict<RegularSimpleCard> thisList)
        {
            if (thisList.Count < 2)
            {
                ToastPlatform.ShowError("Must have at least 2 cards for attacking");
                return false;
            }
            if (thisList.Any(items => items.Color == EnumRegularColorList.Black))
            {
                ToastPlatform.ShowError("Cannot attack with black (defense) cards");
                return false;
            }
            if (thisList.Count > 3)
            {
                ToastPlatform.ShowInfo("Cannot attack with more than 3 cards");
                return false;
            }
            return true;
        }
        public bool CanPlayDefenseCards(IDeckDict<RegularSimpleCard> thisList)
        {
            if (thisList.Count == 0)
            {
                ToastPlatform.ShowError("Must choose at least one card");
                return false;
            }
            if (thisList.Count > 3)
            {
                ToastPlatform.ShowError("Cannot play more than 3 cards for defense");
                return false;
            }
            if (thisList.Any(items => items.Color == EnumRegularColorList.Red))
            {
                ToastPlatform.ShowError("Cannot use attack (red) cards for defense");
                return false;
            }
            return true;
        }
        ...
        {
            SingleInfo = PlayerList!.GetWhoPlayer();
            ToastPlatform.ShowInfo($"{SingleInfo.NickName} has scored a goal");
            SingleInfo.Score++;
