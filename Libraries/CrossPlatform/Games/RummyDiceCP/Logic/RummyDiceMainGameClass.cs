                ...
                if (thisCollection.Count > 0)
                {
                    foreach (var newSet in ThisPhase.PhaseSets)
                    {
                        if (newSet.DidSucceed == false)
                        {
                            ...
                            {
                                EnumWhatSets.Colors => Rummys!.IsNewRummy(thisCollection, newSet.HowMany, EnumRummyType.Colors),
                                EnumWhatSets.Kinds => Rummys!.IsNewRummy(thisCollection, newSet.HowMany, EnumRummyType.Sets),
                                EnumWhatSets.Runs => Rummys!.IsNewRummy(thisCollection, newSet.HowMany, EnumRummyType.Runs),
