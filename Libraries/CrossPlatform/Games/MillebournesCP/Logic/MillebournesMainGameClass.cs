    ...
    public class MillebournesMainGameClass : CardGameClass<MillebournesCardInformation, MillebournesPlayerItem, MillebournesSaveInfo>, IMiscDataNM, IStartNewGame
    {
        ...
        public async Task ProcessCoupeAsync(int deck, int player)
        {
            ...
            if (SingleInfo.PlayerCategory == EnumPlayerCategory.Self)
            {
                ...
                {
                    ToastPlatform.ShowSuccess("You got a coup fourre.  However, you will not get another turn because you have no cards left");
                }
                else
                {
                    ToastPlatform.ShowSuccess("You got a coup fourre.  Please take another turn.");
                }
            }
            else
            {
                if (SingleInfo.MainHandList.Count == 1)
                {
                    ToastPlatform.ShowInfo($"{SingleInfo.NickName} has got a coup fourre.  However, he will not get another turn turn because he has no more cards left");
                }
                else
                {
                    ToastPlatform.ShowInfo($"{SingleInfo.NickName} got a coup fourre.  Therefore, he will get another turn.");
                }
            }
            _isCoupe = true;
            await DrawAsync();
            ...
            if (whichPile == EnumPileType.Safety)
            {
                if (isCoupe == false)
                {
                    if (SingleInfo.PlayerCategory == EnumPlayerCategory.Self)
                    {
                        if (SingleInfo.MainHandList.Count == 0)
                        {
                            ToastPlatform.ShowSuccess("You got a safety.  However, you will not get another turn because you have no more cards left");
                        }
                        else
                        {
                            ToastPlatform.ShowSuccess("You got a safety.  Please take another turn.");
                        }
                    }
                    else
                    {
                        if (SingleInfo.MainHandList.Count == 0)
                        {
                            ToastPlatform.ShowInfo($"{SingleInfo.NickName} has gotten a safety.  However, he will not get another turn because he does not have any more cards left");
                        }
                        else
                        {
                            ToastPlatform.ShowInfo($"{SingleInfo.NickName} has gotten a safety.  Therefore, he will get another turn.");
                        }
                    }
                }
                if (SingleInfo.MainHandList.Count > 0)
